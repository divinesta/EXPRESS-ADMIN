...
| `auth.getCurrentUser` | `(req) => Promise<AdminUser \| null>` | required | Identity adapter. |
...
Mounts, in order: JSON body parser, auth on `/api`, schema route, action routes, CRUD routes, error handler, static UI, SPA fallback.
