...
export const useModelData = (model: Model | undefined, page: number, search: string, filters: Record<string, string>, sort: string, dir: "asc" | "desc") => {
  ...
  useEffect(() => {
    ...
    fetch(`${apiBase}/${model.meta.pluralName}?${params.toString()}`, { credentials: "include", headers: { Accept: "application/json" }, signal: controller.signal })
      .then(async (response) => { if (!response.ok) throw new Error(await readApiError(response)); return response.json() as Promise<{ records: RecordData[]; total: number; totalPages: number }>; })
      .then((payload) => { setRecords(payload.records); setTotal(payload.total); setTotalPages(Math.max(payload.totalPages, 1)); setStatus("ready"); })
      .catch((reason: unknown) => { if (reason instanceof DOMException && reason.name === "AbortError") return; setError(reason instanceof Error ? reason.message : "Could not load records."); setStatus("error"); });
